...
*/
  ...
  for( size_t i = 0; i < nmax; ) {
    ...
    i += i1;
/*
    error = MPI_Sendrecv(  sbuf, m, boost::mpi::get_mpi_datatype<std::complex<double>>(), dest, i
                 , rbuf, m, boost::mpi::get_mpi_datatype<std::complex<double>>(), src, i
                 , world, &status);
*/
//    world.barrier();
  }
  ...
  std::cout << boost::format("rank: %d, elapsed time: %.3f sec, op time: %.5f ms, bandwidth: %.3f GB/s, error: %d")
            %  world.rank() % dt % ( (dt*1000.0)/j ) % ((world.size() - 1)*16.0*m*j/dt/1024.0/1024.0/1024.0) % error
            << std::endl;
