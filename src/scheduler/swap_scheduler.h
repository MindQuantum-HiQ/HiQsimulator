...
class SwapScheduler {
 public:
  ...
  // In worst case works in O(_num_splits * num_gates) time, O(num_gates) memory
  vector<id_num_t> ScheduleSwap();

  SwapScheduler(vector<vector<id_num_t>> _gate, vector<vector<id_num_t>> _gate_ctrl, vector<bool> _gate_diag,
 ...
 private:
  ...
  inline bool CanTake(const int pos, const msk_t cur_locals, const msk_t cur_bad) const {
  ...
  // Fuses i-th single-qubit gate to gate j-th (IMPORTANT: i-th gate has to be removed)
  inline void FuseSingleQubitGateToThis(int i, int j) {
    ...
    if (inter(gate_ctrl[j], gate[i])) {
      gate_ctrl[j] ^= gate[i]; // Remove bit gate[i] from gate_ctrl[j] mask
      gate[j] = unite(gate[j], gate[i]);
    }
  }

  // Fuses i-th single-qubit gate with prev gate containing this qubit (IMPORTANT: in case of success, i-th gate has to be removed)
  ...
  // Fuses i-th gate with next gate containing this qubit (IMPORTANT: in case of success, i-th gate has to be removed)
  ...
  int Rec(int pos, msk_t cur_locals, msk_t cur_bad, int cur_ans, int splits_left);

  // Tries to fuse single qubit gates with next/prev gate containing this qubit.
