     ...
     struct is_nothrow_invocable_if_required_t
         : public
           ...
           std::is_nothrow_invocable<T> /* Note: _r variants not enough to
                                           confirm void return: any return can
                                           be discarded so all returns are
...
template <typename T>
auto make_c_pointer_guard(T*& ptr) noexcept;
template <typename T>
auto make_cpp_pointer_guard(T*& ptr) noexcept;
...
template <typename T>
auto sg::make_c_pointer_guard(T*& ptr) noexcept
{
     return make_scope_guard([&ptr]() noexcept {
          free(ptr);
          ptr = nullptr;
     });
}
template <typename T>
auto sg::make_cpp_pointer_guard(T*& ptr) noexcept
{
     return make_scope_guard([&ptr]() noexcept {
          delete ptr;
          ptr = nullptr;
