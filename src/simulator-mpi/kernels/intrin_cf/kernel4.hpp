...
inline std::size_t get_idx(std::size_t n, std::size_t m0, std::size_t m1, std::size_t m2, std::size_t m3, std::size_t m4)
{
    std::size_t res = (n & m0) ^ ((n & m1) << 1) ^ ((n & m2) << 2) ^ ((n & m3) << 3) ^ ((n & m4) << 4);
    return res;
}

template <class V>
inline void kernel_compute(V &psi, std::size_t I, std::size_t d0, std::size_t d1, std::size_t d2, std::size_t d3, const __m256d m[], const __m256d mt[], std::size_t m0, std::size_t m1, std::size_t m2, std::size_t m3, std::size_t m4)
{
    __m256d v;
    ...
    for (unsigned i = 0; i < buf_sz; ++i) {
        ...
        tmp[2 * buf_sz + i] = add(tmp[2 * buf_sz + i], mul(v, m[104], mt[104]));
        tmp[3 * buf_sz + i] = add(tmp[3 * buf_sz + i], mul(v, m[108], mt[108]));
        tmp[4 * buf_sz + i] = add(tmp[4 * buf_sz + i], mul(v, m[112], mt[112]));
        tmp[5 * buf_sz + i] = add(tmp[5 * buf_sz + i], mul(v, m[116], mt[116]));
        tmp[6 * buf_sz + i] = add(tmp[6 * buf_sz + i], mul(v, m[120], mt[120]));
        tmp[7 * buf_sz + i] = add(tmp[7 * buf_sz + i], mul(v, m[124], mt[124]));
    }
    for (unsigned i = 0; i < buf_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d2 + d0]);
        tmp[i] = add(tmp[i], mul(v, m[97], mt[97]));
        tmp[buf_sz + i] = add(tmp[buf_sz + i], mul(v, m[101], mt[101]));
        tmp[2 * buf_sz + i] = add(tmp[2 * buf_sz + i], mul(v, m[105], mt[105]));
        tmp[3 * buf_sz + i] = add(tmp[3 * buf_sz + i], mul(v, m[109], mt[109]));
        tmp[4 * buf_sz + i] = add(tmp[4 * buf_sz + i], mul(v, m[113], mt[113]));
        tmp[5 * buf_sz + i] = add(tmp[5 * buf_sz + i], mul(v, m[117], mt[117]));
        tmp[6 * buf_sz + i] = add(tmp[6 * buf_sz + i], mul(v, m[121], mt[121]));
        tmp[7 * buf_sz + i] = add(tmp[7 * buf_sz + i], mul(v, m[125], mt[125]));
    }
    for (unsigned i = 0; i < buf_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d2 + d1]);
        tmp[i] = add(tmp[i], mul(v, m[98], mt[98]));
        tmp[buf_sz + i] = add(tmp[buf_sz + i], mul(v, m[102], mt[102]));
        tmp[2 * buf_sz + i] = add(tmp[2 * buf_sz + i], mul(v, m[106], mt[106]));
        tmp[3 * buf_sz + i] = add(tmp[3 * buf_sz + i], mul(v, m[110], mt[110]));
        tmp[4 * buf_sz + i] = add(tmp[4 * buf_sz + i], mul(v, m[114], mt[114]));
        tmp[5 * buf_sz + i] = add(tmp[5 * buf_sz + i], mul(v, m[118], mt[118]));
        tmp[6 * buf_sz + i] = add(tmp[6 * buf_sz + i], mul(v, m[122], mt[122]));
        tmp[7 * buf_sz + i] = add(tmp[7 * buf_sz + i], mul(v, m[126], mt[126]));
    }
    for (unsigned i = 0; i < buf_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d2 + d1 + d0]);
        tmp[i] = add(tmp[i], mul(v, m[99], mt[99]));
        tmp[buf_sz + i] = add(tmp[buf_sz + i], mul(v, m[103], mt[103]));
        tmp[2 * buf_sz + i] = add(tmp[2 * buf_sz + i], mul(v, m[107], mt[107]));
        tmp[3 * buf_sz + i] = add(tmp[3 * buf_sz + i], mul(v, m[111], mt[111]));
        tmp[4 * buf_sz + i] = add(tmp[4 * buf_sz + i], mul(v, m[115], mt[115]));
        tmp[5 * buf_sz + i] = add(tmp[5 * buf_sz + i], mul(v, m[119], mt[119]));
        tmp[6 * buf_sz + i] = add(tmp[6 * buf_sz + i], mul(v, m[123], mt[123]));
        tmp[7 * buf_sz + i] = add(tmp[7 * buf_sz + i], mul(v, m[127], mt[127]));
    }
    for (unsigned i = 0; i < buf_sz; ++i) {
    ...
    for (unsigned i = 0; i < buf_sz; ++i) {
        store(&psi[idxes[i] + d2], &psi[idxes[i] + d2 + d0], tmp[2 * buf_sz + i]);
    }
    for (unsigned i = 0; i < buf_sz; ++i) {
        store(&psi[idxes[i] + d2 + d1], &psi[idxes[i] + d2 + d1 + d0], tmp[3 * buf_sz + i]);
    }
    for (unsigned i = 0; i < buf_sz; ++i) {
        store(&psi[idxes[i] + d3], &psi[idxes[i] + d3 + d0], tmp[4 * buf_sz + i]);
    }
    for (unsigned i = 0; i < buf_sz; ++i) {
        store(&psi[idxes[i] + d3 + d1], &psi[idxes[i] + d3 + d1 + d0], tmp[5 * buf_sz + i]);
    }
    for (unsigned i = 0; i < buf_sz; ++i) {
        store(&psi[idxes[i] + d3 + d2], &psi[idxes[i] + d3 + d2 + d0], tmp[6 * buf_sz + i]);
    }
    for (unsigned i = 0; i < buf_sz; ++i) {
        store(&psi[idxes[i] + d3 + d2 + d1], &psi[idxes[i] + d3 + d2 + d1 + d0], tmp[7 * buf_sz + i]);
    }
}

template <class V>
inline void kernel_compute(V &psi, std::size_t I, std::size_t d0, std::size_t d1, std::size_t d2, std::size_t d3, const __m256d m[], const __m256d mt[], std::size_t m0, std::size_t m1, std::size_t m2, std::size_t m3, std::size_t m4, std::size_t cmask)
{
    __m256d v;
    __m256d tmp[buf_sz * 8];
    size_t idxes[buf_sz];
    ...
    for (unsigned i = 0; i < real_sz; ++i) {
        ...
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[13], mt[13]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[17], mt[17]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[21], mt[21]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[25], mt[25]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[29], mt[29]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d1]);
        tmp[i] = add(tmp[i], mul(v, m[2], mt[2]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[6], mt[6]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[10], mt[10]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[14], mt[14]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[18], mt[18]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[22], mt[22]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[26], mt[26]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[30], mt[30]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d1 + d0]);
        tmp[i] = add(tmp[i], mul(v, m[3], mt[3]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[7], mt[7]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[11], mt[11]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[15], mt[15]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[19], mt[19]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[23], mt[23]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[27], mt[27]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[31], mt[31]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d2]);
        tmp[i] = add(tmp[i], mul(v, m[32], mt[32]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[36], mt[36]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[40], mt[40]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[44], mt[44]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[48], mt[48]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[52], mt[52]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[56], mt[56]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[60], mt[60]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d2 + d0]);
        tmp[i] = add(tmp[i], mul(v, m[33], mt[33]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[37], mt[37]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[41], mt[41]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[45], mt[45]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[49], mt[49]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[53], mt[53]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[57], mt[57]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[61], mt[61]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d2 + d1]);
        tmp[i] = add(tmp[i], mul(v, m[34], mt[34]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[38], mt[38]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[42], mt[42]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[46], mt[46]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[50], mt[50]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[54], mt[54]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[58], mt[58]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[62], mt[62]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d2 + d0 + d1]);
        tmp[i] = add(tmp[i], mul(v, m[35], mt[35]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[39], mt[39]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[43], mt[43]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[47], mt[47]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[51], mt[51]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[55], mt[55]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[59], mt[59]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[63], mt[63]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d3]);
        tmp[i] = add(tmp[i], mul(v, m[64], mt[64]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[68], mt[68]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[72], mt[72]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[76], mt[76]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[80], mt[80]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[84], mt[84]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[88], mt[88]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[92], mt[92]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d0]);
        tmp[i] = add(tmp[i], mul(v, m[65], mt[65]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[69], mt[69]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[73], mt[73]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[77], mt[77]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[81], mt[81]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[85], mt[85]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[89], mt[89]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[93], mt[93]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d1]);
        tmp[i] = add(tmp[i], mul(v, m[66], mt[66]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[70], mt[70]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[74], mt[74]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[78], mt[78]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[82], mt[82]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[86], mt[86]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[90], mt[90]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[94], mt[94]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d0 + d1]);
        tmp[i] = add(tmp[i], mul(v, m[67], mt[67]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[71], mt[71]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[75], mt[75]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[79], mt[79]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[83], mt[83]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[87], mt[87]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[91], mt[91]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[95], mt[95]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d2]);
        tmp[i] = add(tmp[i], mul(v, m[96], mt[96]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[100], mt[100]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[104], mt[104]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[108], mt[108]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[112], mt[112]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[116], mt[116]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[120], mt[120]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[124], mt[124]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d2 + d0]);
        tmp[i] = add(tmp[i], mul(v, m[97], mt[97]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[101], mt[101]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[105], mt[105]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[109], mt[109]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[113], mt[113]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[117], mt[117]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[121], mt[121]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[125], mt[125]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d2 + d1]);
        tmp[i] = add(tmp[i], mul(v, m[98], mt[98]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[102], mt[102]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[106], mt[106]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[110], mt[110]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[114], mt[114]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[118], mt[118]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[122], mt[122]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[126], mt[126]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        v = load2(&psi[idxes[i] + d3 + d2 + d1 + d0]);
        tmp[i] = add(tmp[i], mul(v, m[99], mt[99]));
        tmp[real_sz + i] = add(tmp[real_sz + i], mul(v, m[103], mt[103]));
        tmp[2 * real_sz + i] = add(tmp[2 * real_sz + i], mul(v, m[107], mt[107]));
        tmp[3 * real_sz + i] = add(tmp[3 * real_sz + i], mul(v, m[111], mt[111]));
        tmp[4 * real_sz + i] = add(tmp[4 * real_sz + i], mul(v, m[115], mt[115]));
        tmp[5 * real_sz + i] = add(tmp[5 * real_sz + i], mul(v, m[119], mt[119]));
        tmp[6 * real_sz + i] = add(tmp[6 * real_sz + i], mul(v, m[123], mt[123]));
        tmp[7 * real_sz + i] = add(tmp[7 * real_sz + i], mul(v, m[127], mt[127]));
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        store(&psi[idxes[i]], &psi[idxes[i] + d0], tmp[i]);
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        store(&psi[idxes[i] + d1], &psi[idxes[i] + d1 + d0], tmp[real_sz + i]);
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        store(&psi[idxes[i] + d2], &psi[idxes[i] + d2 + d0], tmp[2 * real_sz + i]);
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        store(&psi[idxes[i] + d2 + d1], &psi[idxes[i] + d2 + d1 + d0], tmp[3 * real_sz + i]);
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        store(&psi[idxes[i] + d3], &psi[idxes[i] + d3 + d0], tmp[4 * real_sz + i]);
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        store(&psi[idxes[i] + d3 + d1], &psi[idxes[i] + d3 + d1 + d0], tmp[5 * real_sz + i]);
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        store(&psi[idxes[i] + d3 + d2], &psi[idxes[i] + d3 + d2 + d0], tmp[6 * real_sz + i]);
    }
    for (unsigned i = 0; i < real_sz; ++i) {
        store(&psi[idxes[i] + d3 + d2 + d1], &psi[idxes[i] + d3 + d2 + d1 + d0], tmp[7 * real_sz + i]);
    }
}


template <class V, class M>
inline void kernel_core(V &v, unsigned id3, unsigned id2, unsigned id1, unsigned id0, M const& m, std::size_t ctrlmask)
{
    std::size_t n = v.size();
    ...
    if (ctrlmask == 0) {
        ...
        #pragma omp for collapse(1) schedule(static)
        for (std::size_t i = 0; i < n >> 4; i += buf_sz) {
            kernel_compute(v, i, d0, d1, d2, d3, mm, mmt, m0, m1, m2, m3, m4, ctrlmask);
        }
    }
}

template <class V, class M, void K(V &, unsigned, unsigned, unsigned, unsigned, M const&, std::size_t)>
inline void kernelK(V &v, unsigned id3, unsigned id2, unsigned id1, unsigned id0, M const& m, std::size_t ctrlmask)
{
