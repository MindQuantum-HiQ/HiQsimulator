...
SimulatorMPI::SimulatorMPI(uint64_t seed, size_t max_local, size_t max_cluster_size): SimulatorMPI(mpi::communicator(), seed, max_local, max_cluster_size)
{
}

SimulatorMPI::SimulatorMPI(mpi::communicator aWorld, uint64_t seed, size_t max_local, size_t max_cluster_size)
      ...
      buffs_(4)
{
  ...
  VLOG(0) << format("ctor(): rank = %d; seed = %u; max_local = %d; max_cluster_size = %d")
...
SimulatorMPI::~SimulatorMPI() {
  ...
  VLOG(0) << format("     *** TOTAL STATS ***     ");
  VLOG(0) << format(" Time                   %.3lf") % total_duration;
  VLOG(0) << format("   -- Runs              %.3lf") % total_runs_duration;
  VLOG(0) << format("   -- Swaps             %.3lf") % total_swap_duration;
  VLOG(0) << format("   -- Measures          %.3lf") % total_measure_duration;
  VLOG(0) << format("   -- Allocations       %.3lf") % total_alloc_duration;
  VLOG(0) << format("   -- Deallocations     %.3lf") % total_dealloc_duration;
  VLOG(0) << format(" Gates                  %d") % total_gates;
  VLOG(0) << format(" Runs                   %d") % total_runs;
  VLOG(0) << format(" Stages                 %d") % total_stages;
  VLOG(0) << format(" Gates/run              %.3lf") % (static_cast<Float>(total_gates) / total_runs);
  VLOG(0) << format(" Time/run               %.3lf") % (total_runs_duration / total_runs);
  VLOG(0) << format(" Gates/stage            %.3lf") % (static_cast<Float>(total_gates) / total_stages);
  VLOG(0) << format(" Runs/stage             %.3lf") % (static_cast<Float>(total_runs) / total_stages);
}

template<class V>
inline void FillVector(typename V::iterator start, typename V::iterator end, const typename V::value_type v) {
...
inline void SimulatorMPI::CheckNorm() {
  ...
  VLOG(4) << format("CheckNorm(): norm = %.3lf; local_norm = %.3lf") % norm % local_norm;
  VLOG(4) << format("CheckNorm(): local state vector: ") << print(vec_);
...
void SimulatorMPI::AllocateLocalQubit(Index id) {
  VLOG(1) << format("AllocateLocalQubit(): id = %u; bit = %u") % id % locals_.size();
...
void SimulatorMPI::AllocateGlobalQubit(Index id) {
  ...
  VLOG(1) << format("AllocateGlobalQubit(): id = %u; bit = %u") % id % pos;
  globals_[pos] = id;
}

void SimulatorMPI::AllocateQubit(Index id) {
  auto start_alloc_time = Clock::now();
  VLOG(1) << format("AllocateQubit(): id = %u") % id;
  ...
  if (local_size < kMinLocal_) {
    ...
    auto message =
        (format("AllocateQubit(): can't allocate more than %u qubits") % (globals_.size() + locals_.size())).str();
    LOG(ERROR) << message;
    world_.barrier();
    throw std::runtime_error(message);
  }
...
void SimulatorMPI::AllocateQureg(const std::vector<Index> &ids, Complex init) {
    VLOG(1) << format("AllocateQureg(): ids = ") << print(ids);
    VLOG(1) << format("AllocateQureg(): init = %f") % init;

    if (init != 0.0 && !locals_.empty()) {
        auto message = "AllocateQureg(): initialization of only first qureg is supported";
        LOG(ERROR) << message;
        world_.barrier();
        throw std::runtime_error(message);
    }
...
size_t SimulatorMPI::ArrayFindSure(const std::vector<Index> &v, Index val) const {
  ...
  if (pos == kNotFound_) {
    ...
    message << format("ArrayFindSure(): Can't find %u in ") % val << print(v);
...
void SimulatorMPI::DeallocateLocalQubit(Index id) { // TODO make parallel
  VLOG(1) << format("DeallocateLocalQubit(): id = %u") % id;
  ...
  VLOG(1) << format("DeallocateLocalQubit(): norm(0) = %.3lf; norm(1) = %.3lf") % all[0] % all[1];
  if (!((all[0] > kMaxFloatError_) ^ (all[1] > kMaxFloatError_))) {
    auto message = (format("DeallocateLocalQubit(): qubit %u is entangled") % id).str();
    LOG(ERROR) << message;
    world_.barrier();
    throw std::runtime_error(message);
  }
  ...
  if (all[0] > kMaxFloatError_) {
    for (size_t i = (1ul << (pos)); (i << 1) < vec_.size(); i += (1ul << (pos))) {
...
void SimulatorMPI::DeallocateGlobalQubit(Index id) { // TODO make parallel
  VLOG(1) << format("DeallocateGlobalQubit(): id = %u") % id;
  ...
  VLOG(1) << format("DeallocateGlobalQubit(): norm(0) = %.3lf; norm(1) = %.3lf") % all[0] % all[1];
  if (!((all[0] > kMaxFloatError_) ^ (all[1] > kMaxFloatError_))) {
    auto message = (format("DeallocateGlobalQubit(): qubit %u is entangled") % id).str();
    LOG(ERROR) << message;
    world_.barrier();
    throw std::runtime_error(message);
  }
  ...
  if (all[1] > kMaxFloatError_) {
    VLOG(1) << format( "DeallocateGlobalQubit(): deallocating qubit %u in |1> state" ) % id;
  ...
  VLOG(2) << format( "DeallocateGlobalQubit(): qubit %u deallocated" ) % id;
}

void SimulatorMPI::DeallocateQubit(Index id) {
  auto start_dealloc_time = Clock::now();
  VLOG(1) << format("DeallocateQubit(): id = %u") % id;

  auto local_size = locals_.size();
  auto global_size = globals_.size() - std::count(globals_.begin(), globals_.end(), -1);

  if (ArrayFind(locals_, id) != kNotFound_) {
    VLOG(1) << format("DeallocateQubit(): qubit %u is local now") % id;

    if (local_size > kMinLocal_ || global_size == 0) {
      VLOG(1) << "DeallocateQubit(): just deallocate this local qubit";
      DeallocateLocalQubit(id);
    } else {
      size_t i = std::find_if(globals_.begin(), globals_.end(), [](Index id) {
        return (size_t)id != kNotFound_;
      }) - globals_.begin();
      VLOG(1) << format("DeallocateQubit(): swapping with global qubit %u") % globals_[i];
      SwapQubits({globals_[i], id});
      DeallocateGlobalQubit(id);
    }

  } else {
    VLOG(1) << format("DeallocateQubit(): qubit %u is global qubit now") % id;

    if (local_size > kMinLocal_ || global_size == 0) {
      VLOG(1) << format("DeallocateQubit(): swapping with local qubit %u") % locals_.back();
      SwapQubits({id, locals_.back()});
      DeallocateLocalQubit(id);
    } else {
      VLOG(1) << "DeallocateQubit(): just deallocate this global qubit";
      DeallocateGlobalQubit(id);
    }
  }
  ...
  auto dealloc_duration = Duration(Clock::now() - start_dealloc_time).count();
  total_dealloc_duration += dealloc_duration;

}

uint64_t SimulatorMPI::IdsToBits(const std::vector<Index> &ids, const std::vector<Index> &perm) const {
  uint64_t mask = 0;
  for (auto i : ids) {
  ...
  switch (ids.size()) {
    case 0:
      if (m[0][0] != static_cast<StateVector::value_type>(1)) {
        kernelK_diag1<StateVector, StateVector::value_type>(vec_, m[0][0]);
      }
      break;
    case 1:
#pragma omp parallel
      diag ?
      kernelK<StateVector, Matrix, kernel_core_diag>(vec_, ids_pos[0], m, ctrl_mask)
           : kernelK<StateVector, Matrix, kernel_core>(vec_, ids_pos[0], m, ctrl_mask);
      break;
    case 2:
#pragma omp parallel
      diag ?
      kernelK<StateVector, Matrix, kernel_core_diag>(vec_,
                                                                         ids_pos[1],
                                                                         ids_pos[0],
                                                                         m,
                                                                         ctrl_mask)
           : kernelK<StateVector, Matrix, kernel_core>(vec_, ids_pos[1], ids_pos[0], m, ctrl_mask);
      break;
    case 3:
#pragma omp parallel
      diag ?
      kernelK<StateVector, Matrix, kernel_core_diag>(vec_,
                                                                         ...
                                                                         m,
                                                                         ctrl_mask)
           : kernelK<StateVector, Matrix, kernel_core>(vec_, ids_pos[2], ids_pos[1], ids_pos[0], m, ctrl_mask);
      break;
    case 4:
#pragma omp parallel
      diag ?
      kernelK<StateVector, Matrix, kernel_core_diag>(vec_,
                                                                         ...
                                                                         m,
                                                                         ctrl_mask)
           : kernelK<StateVector, Matrix, kernel_core>(vec_,
                                                       ids_pos[3],
                                                       ids_pos[2],
                                                       ids_pos[1],
                                                       ids_pos[0],
...
#pragma omp parallel
      ...
      break;
    default:
      auto message = (format("Run(): cannot apply %u qubits gate") % ids.size()).str();
      LOG(ERROR) << message;
      world_.barrier();
      throw std::runtime_error(message);
  }
  ...
  VLOG(2) << format("Run(): duration = %.3lf") % run_duration;
...
std::tuple<std::map<int, int>, SimulatorMPI::StateVector&> SimulatorMPI::cheat_local(){
...
SimulatorMPI::Float SimulatorMPI::GetProbability(const std::vector<bool> &bit_string,
    ...
    return getProbability_internal(local_msk, local_val, global_msk, global_val);
}

SimulatorMPI::Complex SimulatorMPI::GetAmplitude(const std::vector<bool> &bit_string,
  ...
  auto qureg_size = locals_.size() + globals_.size() - count(globals_.begin(), globals_.end(), kNotFound_);
  ...
  if( (1ul << qureg_size) - 1 != check ) {
    auto message = "GetAmplitude(): the second argument must be a permutation of all allocated qubits.";
    LOG(ERROR) << message;
    world_.barrier();
    throw std::runtime_error(message);
  }
...
std::vector<SimulatorMPI::Index> SimulatorMPI::ExtractLocalCtrls(const std::vector<Index> &ctrl) const {
...
void SimulatorMPI::ApplyGate(Matrix m, std::vector<Index> ids, std::vector<Index> ctrls) {
  ...
  if (global_id_mask != 0 && !diag) {
    auto message = "ApplyGate(): can't apply non-diagonal gate to global qubits";
    LOG(ERROR) << message;
    world_.barrier();
    throw std::runtime_error(message);
  }
  ...
  ctrls.clear();
  ids.erase(std::remove_if(ids.begin(), ids.end(), [&](const Index id) {
...
void SimulatorMPI::calcLocalApproxDistribution(const size_t n) {
  ...
  FillVector<bc::vector<Float>>(block_distribution.begin(), block_distribution.end(), 0.0);
...
#pragma omp distribute parallel for
  ...
  std::partial_sum(block_distribution.begin(),
                   block_distribution.end(),
                   block_distribution.begin());
}

SimulatorMPI::Float
SimulatorMPI::getProbability_internal(uint64_t local_msk, uint64_t local_val, uint64_t global_msk, uint64_t global_val) {
  VLOG(4) << format("getProbability_internal(): local_msk: %d, local_val: %d, global_msk: %d, global_val: %d") % local_msk % local_val % global_msk % global_val;
  VLOG(4) << format("getProbability_internal(): local state vector: ") << print(vec_);
...
#pragma omp parallel for reduction(+:local_probability) schedule(static)
  ...
  Float probability = mpi::all_reduce(world_, local_probability, std::plus<Float>());
  VLOG(1) << format("getProbability_internal(): local_probability = %.3lf; probability = %.3lf") % local_probability % probability;

  return probability;
}

void SimulatorMPI::normalize(Float norm, uint64_t local_msk, uint64_t local_val, uint64_t global_msk, uint64_t global_val) {
...
std::vector<bool> SimulatorMPI::MeasureQubits(std::vector<Index> const &ids) {
  ...
  mpi::all_gather(world_, block_distribution.data(), static_cast<int>(n), total_block_distribution.data());
  ...
  VLOG(1) << format("MeasureQubits(): rnd = %.3lf; src_rank: %u, src_index: %u") % rnd % src_rank % src_index;

  auto res = std::vector<bool>(ids.size());
  if (static_cast<int>(src_rank) == world_.size()) {
    auto message = (format("MeasureQubits(): Random number %.3lf > norm partial sum %.3lf") % rnd
        % total_block_distribution.back()).str();
    LOG(ERROR) << message;
    world_.barrier();
    throw std::runtime_error(message);
  }
  ...
  VLOG(1) << format("MeasureQubits(): res_index: %u") % res_index;

  // determine result vector (boolean values for each qubit)
  // and create local_msk to detect bad entries (i.e., entries that don't agree with measurement)
  uint64_t local_msk = 0;
  uint64_t local_val = 0;
  uint64_t global_msk = 0;
  uint64_t global_val = 0;
  ...
  auto norm = getProbability_internal(local_msk, local_val, global_msk, global_val);

  normalize(norm, local_msk, local_val, global_msk, global_val);

  auto measure_duration = Duration(Clock::now() - start_measure_time).count();
  total_measure_duration += measure_duration;
  VLOG(1) << format("MeasureQubits(): duration = %.3lf") % measure_duration;

  return res;
}

void SimulatorMPI::collapseWaveFunction(const std::vector<Index>& ids, const std::vector<bool>& values) {

    VLOG(1) << format("collapseWaveFunction(): ids: " ) << print(ids);
    VLOG(1) << format("collapseWaveFunction(): values: " ) << print(values);
    ...
    if (norm < 1.e-12) {
        auto message = "collapseWaveFunction(): Invalid collapse! Probability is ~0.";
        LOG(ERROR) << message;
        world_.barrier();
        throw std::runtime_error(message);
    }
...
void SimulatorMPI::SwapQubitsWrapper(const std::vector<Index> &swap_pairs) {
  ...
  auto bandwidth = (Float{1} / (1ul << 30)) * swapped_bytes * 8 / swap_duration;
  VLOG(1) << format("SwapQubitsWrapper(): duration = %.3lf; qubits = %d; bandwidth = %.3lf Gb/s") % swap_duration
      % (swap_pairs.size() / 2) % bandwidth;
...
void SimulatorMPI::SwapQubits(const std::vector<Index> &swap_pairs) {
  ...
  VLOG(3) << format("SwapQubits(): color = %d; comm.size() = %d") % color % comm.size();
...
void SimulatorMPI::EndStage() {
  ...
  VLOG(1) << format("EndStage(): duration = %.3lf; gates = %d; runs = %d; gates/run = %.3lf; time/run = %.3lf")
      % stage_duration % stage_gates % stage_runs % (stage_gates / static_cast<Float>(stage_runs))
