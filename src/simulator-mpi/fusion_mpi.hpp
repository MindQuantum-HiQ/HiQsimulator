...
public:
    ...
    Item(Matrix mat, uint64_t aFlags, IndexVector idx) : mat_(mat), idx_(idx), m_flags (aFlags)
    {}
...
public:
    ...
    void insert(Matrix matrix, uint64_t flags, IndexVector index_list, IndexVector const& ctrl_list = {})
    {
        if( matrix.size() == 1 ) {
            factor *= matrix[0][0];
            return;
    ...
    void perform_fusion(Matrix& fused_matrix, IndexVector& index_list, IndexVector& ctrl_list, uint64_t& out_flags){
        ...
        DLOG(INFO) << format("perform_fusion(): items_.size(): %d, num qubits: %d") % items_.size() % N;
