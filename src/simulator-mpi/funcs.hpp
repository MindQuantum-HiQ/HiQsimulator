...
void printAmplitude(const Simulator& sim, std::vector<bool>&& bits_string, size_t bit) {
    if(bit != 0) {
        bits_string[bit] = 0;
...
void printAmplitudes(const Simulator& sim) {
    printAmplitude(sim, std::vector<bool>(sim.TotalQubitsCount()), sim.TotalQubitsCount()-1);
}
}
...
template<class T, class T2>
std::vector<T> permuteQubits(std::vector<T>&& aPerm, const std::vector<T2>& aPairs) {
...
template<class T>
std::vector<typename T::value_type> applyPermutation(const T& aV, const std::vector<uint8_t>& aPerm) {
...
std::vector<T> q2bits(uint64_t aM, const std::vector<T1>& aQbitPairs, const M& anInversePerm) {
    std::map<T1,T1> m;
    for(size_t i = 0; i < aQbitPairs.size(); i += 2) {
        m[anInversePerm[aQbitPairs[i+1]]] = anInversePerm[aQbitPairs[i]] - aM;
    }
...
void q2bits_ng(std::vector<T>& res, const std::vector<T1>& aQbitPairs, M pos) {
    ...
    std::sort( res.begin() + 3*n_bits, res.begin() + 4*n_bits, [&](size_t i1, size_t i2) { return res[i1] < res[i2]; } );

    //sort local bits, so the above permutation corresponds to this sorted order
...
template<class T>
T getColor_ng(uint64_t M, T aRank, const std::vector<T>& aQubitPairs, const bc::vector<T>& iperm) {
...
std::string bitstring( T val, uint64_t L ) {
    return std::bitset<sizeof(T)*8>(val).to_string().substr(sizeof(T)*8 - L, L);
}

template<class Stream, class T>
void printStateVector(Stream& out, const char* name, uint64_t L, const std::vector<T>& aSV) {
    for(size_t i = 0; i < aSV.size(); ++i) {
        out << format("%5d: %s") % i % bitstring(aSV[i], L)
            << endl;
    }
}
