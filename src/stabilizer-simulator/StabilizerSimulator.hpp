...
#define QUBIT_ALLOCATE_CHECK(qubit_id, gate_name)			\
	...
	THROW_RUNTIME_ERROR("Error (" gate_name "): Qubit ID is not "	\
...
#pragma omp parallel for schedule(static, 32)
	    ...
	    auto n(num_qubits_);
	    std::generate(begin(available_qubits_),
			  end(available_qubits_),
			  [&n] () { return --n; });
#else
	    std::generate(begin(available_qubits_),
			  end(available_qubits_),
    ...
    void allocate_qubit(unsigned qubit_id)
	{
		...
		if (available_qubits_.empty()) {
		    THROW_RUNTIME_ERROR("AllocateQubit: cannot allocate Qubit. "
		...
		THROW_RUNTIME_ERROR("AllocateQubit: ID already exists. "
    ...
    void deallocate_qubit(unsigned qubit_id)
	{
		...
		THROW_RUNTIME_ERROR("Error: Qubit has not been measured / "
    ...
    std::vector<bool>
    measure_qubits_return(std::vector<unsigned> const& qubit_ids)
	{
	    sync();
	    std::vector<bool> ret;
	    std::transform(
    ...
    bool get_classical_value(unsigned qubit_id, calc_type tol = 1.e-12)
	{
		...
		THROW_RUNTIME_ERROR("Qubit has not been measured / "
			   ...
			   const std::vector<unsigned>& qubit_ids)
	{
	    ...
	    return get_probability_recursive_(begin(bitstring),
					      end(bitstring),
					      begin(qubit_ids),
					      end(qubit_ids));
	}
		    ...
		    const std::vector<bool>& bitstring)
	{
		...
		if (set_qubits_(allocated_qubits_[*qubit_it],
				      *bitstring_it)) {
...
#pragma omp parallel for schedule(static, 32)
		...
		for (auto d = 0U; d < gates_.max_d; ++d) {
		    h_and_s_kernel_(begin(gates_.H) + d*nmasks_8,
				    ...
				    begin(gates_.S) + (d+1)*nmasks_8,
				    stab);
		    for (const auto& cnot_tuple : gates_.C[d]) {
			cnot_kernel_(cnot_tuple.first, cnot_tuple.second, stab);
		    }
		}
	    }
	    gates_.reset();
...
private:
    ...
    void toggle_stab_z_(unsigned stab,
			unsigned qubit,
    ...
    unsigned compute_phase_(unsigned stabilizer_h, unsigned stabilizer_i) const
	{
		...
		auto q = _mm256_and_si256(_mm256_and_si256(
					      _mm256_or_si256(xi, zh),
		...
		exp_high1 = _mm256_xor_si256(_mm256_and_si256(exp_low, r_and_s),
					     exp_high1);
		exp_high2 = _mm256_xor_si256(exp_high2,
					     _mm256_and_si256(q, r_and_s));
		...
		*/
	    ...
	    return (
		((exponent + 2 * get_phase_bit_(stabilizer_h)
		  + 2 * get_phase_bit_(stabilizer_i)) % 4) + 4) % 4;
	}
    ...
    void rowsum_(unsigned stabilizer_h, unsigned stabilizer_i)
	{
	    ...
	    // 1 (-1) as before:
	    set_phase_bit_(stabilizer_h, compute_phase_(stabilizer_h,
							stabilizer_i) / 2);
				...
				unsigned stab)
	{
	    ...
	    for (auto h_it(Hmasks_begin), s_it(Smasks_begin);
		 h_it < Hmasks_end;
		 h_it += 8, s_it += 8, i += 8) {
	...
	{
	    auto pos = std::max(pos_[control_qubit_index],
				pos_[target_qubit_index]);
...
#pragma omp parallel for schedule(static, 32)
    ...
    void pre_measurement_deterministic_(unsigned qubit_index)
	{
	    ...
	    std::fill(begin(x_) + start,
		      begin(x_) + start + row_length_,
		      0);
	    std::fill(begin(z_) + start,
	...
	std::vector<unsigned>::const_iterator qubit_ids_end)
	{
		...
		if (bitstring_begin != bitstring_end && prob != 0.) {
		    prob = get_probability_recursive_(bitstring_begin,
						      bitstring_end,
						      qubit_ids_begin,
						      qubit_ids_end);
		}
    ...
    void collapse_wavefunction_(unsigned qubit_index, bool state)
	{
		...
		if (get_phase_bit_(2 * num_qubits_) != state) {
		    THROW_RUNTIME_ERROR("collapse_wavefunction(): "
