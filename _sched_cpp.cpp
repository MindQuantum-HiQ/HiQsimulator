...
PYBIND11_MODULE(_sched_cpp, m) {
    py::class_<SwapScheduler>(m, "SwapScheduler")
        .def(py::init<vector<vector<id_num_t>>, vector<vector<id_num_t>>, vector<bool>, int, int, bool>())
        .def("ScheduleSwap", &SwapScheduler::ScheduleSwap)
        ;
    py::class_<ClusterScheduler>(m, "ClusterScheduler")
        .def(py::init<vector<vector<id_num_t>>, vector<vector<id_num_t>>, vector<bool>, vector<id_num_t>, vector<id_num_t>, int>())
