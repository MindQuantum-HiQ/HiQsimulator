...
"""
Tests for projectq.backends._sim._simulator_mpi.py, using both the Python
...
from hiq.projectq.cengines import HiQMainEngine, GreedyScheduler
...
def get_available_simulators():
    result = []
    try:
        from hiq.projectq.backends import SimulatorMPI as _
        result.append("hiq_simulator_mpi")
    except ImportError:
        # The HiQ MPI simulator was either not installed or is misconfigured. Skip.
        pass
    return result
...
#
#    # check that SimulatorMPI doesn't support BasicMathGate
#    BasicMathGate(lambda x: x) | qubit
#    cmd = backend.received_commands[-1]
#    assert not sim.is_available(cmd)
#
...
def test_simulator_functional_entangle(sim):
    ...
    assert .5 == pytest.approx(abs(vec[31])**2)  # are never moved even if qubit reordering made
    for i in range(1, 31):
