...
uint64_t calcSwaps(uint64_t aL, uint64_t aM, std::vector<uint64_t> anInvPerm,  std::vector<uint64_t> aPairs)
{
	uint64_t res = 0;
	...
	for(uint64_t rank = 0; rank < (1ul << (aL - aM)); ++rank) {
		...
		for(uint64_t index = 0; index < (1ul << aM); ++index) {
			...
			for(size_t pi = 0; pi < aPairs.size(); pi += 2) {
				...
				uint64_t remote_bit = (1ul << (anInvPerm[remote_qubit] - aM));
			...
			if(new_rank != rank) {
				res += new_index;
//				cout << format("(%d: %d)->(%d: %d)") % rank % index % new_rank % new_index << endl;
			}
		}

		auto end = std::chrono::high_resolution_clock::now();
		std::chrono::duration<double, std::milli> elapsed_us = end-start;
	    std::cout << format("rank: %d, ") % rank << format("elapsed time: %.3f ms") % elapsed_us.count() << endl;
	}
...
uint64_t calcSwaps1(uint64_t aL, uint64_t aM, std::vector<uint64_t> aSwapBits)
{
	...
	for(uint64_t rank = 0; rank < (1ul << (aL - aM)); ++rank) {
		...
		for(uint64_t index = 0; index < (1ul << aM); ++index) {
			...
			if(new_rank != rank)
            {
				cout << format("(%d: %d)->(%d: %d),  dst_idx: %d") % rank % index % new_rank % new_index % dst_idx << endl;
			}
		}
