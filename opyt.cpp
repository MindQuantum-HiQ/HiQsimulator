...
inline void kernel_core_print_idx(V &psi, std::size_t I, std::size_t d0, Mat const& m)
{
...
template <class V, class Mat>
inline void kernel_core_print_idx(V &psi, std::size_t I, std::size_t d0, std::size_t d1, Mat const& m)
{
...
template <class V, class Mat>
inline void kernel_core_set_psi(V &psi, std::size_t I, std::size_t d0, std::size_t d1, Mat const& m)
{
//     std::complex<double> v[4];
//     v[0] = psi[I];
//     v[1] = psi[I + d0];
...
template<size_t n_bits, class V, class Mat, void K(V &, std::size_t, std::size_t, std::size_t, Mat const&)>
void iterate_state_vector(uint64_t M, V& vec, const Mat& m, const std::vector<uint64_t>& trgts, const std::vector<std::pair<uint64_t,bool>>& bits) {
    for(size_t free_idx = 0; free_idx < (1ul << (M - n_bits)); ++free_idx) {
        ...
        for(size_t bit_idx = 0; bit_idx < n_bits; ++bit_idx ) {
            ...
            res_start_idx = 2*r + l;
//             cout << format("bit: %2d, res_start_idx: %s") % bits[bit_idx].first % bitstring(res_start_idx, M) << endl;
            res_start_idx |= bits[bit_idx].second*b;
//             cout << format("         res_start_idx: %s") % bitstring(res_start_idx, M) << endl;
        }
...
void apply_gate(StateVector& vec_, const Fusion::Matrix& m, const std::vector<uint64_t>& trgts, uint64_t ctrlmask) {
    auto t0 = std::chrono::high_resolution_clock::now();

    switch (trgts.size()){
        case 1:
            #pragma omp parallel
            intrin::kernelK<StateVector, Fusion::Matrix, intrin::kernel_core>(vec_, trgts[0], m, ctrlmask);
            break;
        case 2:
            #pragma omp parallel
            intrin::kernelK<StateVector, Fusion::Matrix, intrin::kernel_core>(vec_, trgts[1], trgts[0], m, ctrlmask);
            break;
        case 3:
            #pragma omp parallel
            intrin::kernelK<StateVector, Fusion::Matrix, intrin::kernel_core>(vec_, trgts[2], trgts[1], trgts[0], m, ctrlmask);
            break;
        case 4:
            #pragma omp parallel
            intrin::kernelK<StateVector, Fusion::Matrix, intrin::kernel_core>(vec_, trgts[3], trgts[2], trgts[1], trgts[0], m, ctrlmask);
            break;
        case 5:
            #pragma omp parallel
            intrin::kernelK<StateVector, Fusion::Matrix, intrin::kernel_core>(vec_, trgts[4], trgts[3], trgts[2], trgts[1], trgts[0], m, ctrlmask);
            break;
    }

    auto t1 = std::chrono::high_resolution_clock::now();
    std::chrono::duration<double, std::milli> dt = t1 - t0;
    cout << format("trgts: %d dt: %.3f ms") % trgts.size() % dt.count()  << endl;
}
...
//              std::cout << std::endl << "trgs: " << print(trgts) << std::endl;
...
void orig_algo(uint64_t L, uint64_t M, std::vector<uint64_t>& trgts, std::vector<uint64_t>& ctrls)
{
    StateVector vec_(1ul << M);
    Fusion::Matrix m = construct_gate(trgts.size());
...
int new_algo(uint64_t L, uint64_t M, std::vector<uint64_t>& trgts, std::vector<uint64_t>& ctrls) {
    ...
    std::sort(bits.begin(), bits.end(),
              [](std::pair<uint64_t,bool>& a, std::pair<uint64_t, bool>& b) -> bool { return a.first < b.first; } );
...
//     }
    ...
    switch(trgts.size()) {
        case 2:
            iterate_state_vector<2, StateVector, Fusion::Matrix, kernel_core_set_psi>(M, vec, m, trgts, bits);
            break;
    }
...
//         switch(trgts.size()) {
//             case 1:
// //                 cout << format("i0: %s") % bitstring( res_start_idx, M) << endl;
// //                 cout << format("i1: %s") % bitstring( res_start_idx + (1ul << trgts[0]), M) << endl;
//                 kernel_core_print_idx(vec, res_start_idx, 1ul << trgts[0], m);
//                 break;
//             case 2:
//                 kernel_core_print_idx(vec, res_start_idx, 1ul << trgts[0], 1ul << trgts[1], m);
// //                 cout << format("i0: %s") % bitstring( res_start_idx, M) << endl;
// //                 cout << format("i1: %s") % bitstring( res_start_idx + (1ul << trgts[0]), M) << endl;
// //                 cout << format("i2: %s") % bitstring( res_start_idx + (1ul << trgts[1]), M) << endl;
// //                 cout << format("i3: %s") % bitstring( res_start_idx + (1ul << trgts[0]) + (1ul << trgts[1]), M) << endl;
//                 break;
//         }
//     }
